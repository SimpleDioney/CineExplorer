const API_KEY = "4ea270f32fe4e8fcdfd68b4cd5a7074f";
      ...
      const translations = {
        "en-US": {
          ...
          moodSubtitle:
            "Select your mood for personalized movie recommendations",
          happy: "😊 Happy",
          sad: "😢 Sad",
          anxious: "😰 Anxious",
          bored: "😑 Bored",
      ...
      function updateUI(movie) {
        ...
        document.getElementById("movie-meta").textContent = `${formatDate(
          movie.release_date
        )} | ${formatRuntime(movie.runtime)} | ★ ${movie.vote_average.toFixed(
          1
        )}`;
  ...
  updateSimilarMovies(movie.similar.results);
      ...
      function updateMoodRecommendations(movies, mood) {
        ...
        const movieList =
          moodRecommendationsSection.querySelector(".movie-list");
        const sectionTitle =
      ...
      function updateProviders(providers) {
        const providersContainer = document.getElementById(
          "providers-container"
        );
        ...
        if (
          ...
          const uniqueProviders = allProviders.filter(
            (provider, index, self) =>
              index ===
              self.findIndex((t) => t.provider_id === provider.provider_id)
          );
      ...
      function updateSimilarMovies(similarMovies) {
        const similarMoviesContainer =
          document.getElementById("similar-movies");
      ...
      async function showPopup(actorId) {
        ...
        try {
          ...
          if (actorInfo.birthday) {
            ...
            } ${formatDate(actorInfo.birthday)}</p>`;
          }
          if (actorInfo.deathday) {
            infoHtml += `<p>${
              translations[currentLanguage].deathdate
            } ${formatDate(actorInfo.deathday)}</p>`;
          }
